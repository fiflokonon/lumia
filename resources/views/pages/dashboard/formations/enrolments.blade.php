...
@section('page_title', "Inscriptions à la formation: $formation->title")
<!--**********************************
    Content body start
***********************************-->
@section('content')
                        ...
                        <div class="table-responsive full-data">
                            <table class="table-responsive-lg table display dataTablesCard student-tab dataTable no-footer" id="example-student">
                                ...
                                <tr>
                                    ...
                                    <th>Email</th>
                                    <th>Téléphone</th>
                                    <th>Statut <span class="text-lowercase">de paiement</span></th>
                                    <th>Date <span class="text-lowercase">d'inscription</span></th>
                                    <th class="text-end">Action</th>
                                </tr>
                                </thead>
                                <tbody class="mb-3">

                                @foreach($formation->enrolments as $enrolment)
                                <tr>
                                    <td>
                                        <div class="trans-list">
                                            <img src="/static/akademi/images/trans/10.jpg" alt="" class="avatar me-3">
                                            <h4>{{ $enrolment->user->first_name }} {{ $enrolment->user->last_name }}</h4>
                                        </div>
                                    </td>
                                    <td><span class="text-primary font-w600">{{ $enrolment->user->email }}</span></td>
                                    <td><span class="text-primary font-w600">{{ $enrolment->user->phone }}</span></td>
                                    <td class="text-center">
                                        ...
                                        @else
                                            <div class="badge badge-sm light badge-danger">
                                                Non validé
                                            </div>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="date">{{ \Carbon\Carbon::parse($enrolment->created_at)->locale('fr')->translatedFormat('d F Y')  }}</div>
                                    </td>
                                    <td>
